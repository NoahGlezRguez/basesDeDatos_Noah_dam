-- Con el usuario root
create database empresa2_dam collate utf8mb4_spanish_ci;
-- Con el usuario dam
use empresa2_dam;
...
add constraint fk_coddep foreign key (CodDepDep) references DEPARTAMENTO (CodDep) on delete cascade;

alter table DEPARTAMENTO
add constraint fk_codcen foreign key (CodCen) references CENTRO (CodCen) on delete cascade;
...
add constraint foreign key (CodDep) REFERENCES DEPARTAMENTO (CodDep) ON DELETE CASCADE;
